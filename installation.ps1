...
if (!(Test-Path $OFFLINE_DIR)) {
    ...
    Write-Host "✓ 已创建目录: $OFFLINE_DIR"
} else {
    Write-Host "✓ 目录已存在: $OFFLINE_DIR"
}
...
# 4. 拉取镜像
Write-Host "正在拉取 AstrBot 镜像（约 2GB，请耐心等待）..."
...
Set-Content -Path $COMPOSE_PATH -Value $COMPOSE_CONTENT
...
Set-Content -Path $MOUNT_PATH -Value $MOUNT_SCRIPT
...
if docker ps | grep -q astrbot; then
    ...
    echo -e "${RED}✗ 启动失败，请执行: docker logs astrbot${NC}"
fi
'@
$START_PATH = "$OFFLINE_DIR\start.sh"
Set-Content -Path $START_PATH -Value $START_SCRIPT
...
# 修改 docker-compose.yml 中的数据卷挂载路径（备份原文件）
...
if docker ps | grep -q astrbot; then
    ...
    echo -e "${RED}✗ 安装失败，请执行: docker logs astrbot${NC}"
fi
'@
$INSTALL_PATH = "$OFFLINE_DIR\install.sh"
Set-Content -Path $INSTALL_PATH -Value $INSTALL_SCRIPT
...
Set-Content -Path $STOP_PATH -Value $STOP_SCRIPT

Write-Host "========================================"
Write-Host "✅ 离线包生成完成！"
Write-Host "📁 位置: $OFFLINE_DIR"
Write-Host "📄 包含文件:"
Write-Host "   - astrbot_latest.tar"
Write-Host "   - docker-compose.yml"
Write-Host "   - mount.sh"
Write-Host "   - start.sh (便携模式)"
Write-Host "   - install.sh (安装模式)"
Write-Host "   - stop.sh"
Write-Host "========================================"
Write-Host "下一步："
Write-Host "1. 将整个文件夹复制到 U 盘的 exFAT 分区"
Write-Host "2. 在 Linux 设备上运行: cd /mnt/ventoy_data/AstrBot_Offline"
Write-Host "3. chmod +x *.sh && ./mount.sh"
Write-Host "4. 根据需求选择: ./start.sh (便携) 或 ./install.sh (安装)"